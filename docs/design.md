...
- `ask(requestId, prompt, sites, resumeSessionId?)`：処理中なら `BusyError`。`resumeSessionId` があれば先にタブをその会話へ戻し、それから戦略を実行する(F-03)
...
- 更新時刻を動かすのはターンの追記と会話 URL の保存、つまり**送信したとき**だけとする。名前の変更では動かさない。一覧はこの時刻の降順であり、順序は「最後に使った順」を意味するべきだからである
...
再開が済んだら、**その会話の中身も配信する**。
タブが動いたのに画面が空のままでは、サイドバーの「表示中」の印と食い違うからである。
接続してきたクライアントにも、そのとき開かれている会話を送る(`currentSessionMessage`)。
一方、参照中のまま送信した場合の再開では配信しない。送信直前に画面へ足した入力が、保存済みの内容で上書きされてしまうためである。

**参照中のまま送信した場合も、同じ経路でタブを戻す**。
`prompt` に `resumeSessionId` が付いていれば、`ask` は処理中でないことを確かめた後、現在の会話と違うときだけタブの移動(`resumeTabs`)を行い、そのセッションを現在の会話にしてから送信に進む。
待ち合わせと失敗時の扱いをサーバー側に閉じ込めるための形で、UI は今までどおり一つのメッセージを送るだけでよい。
指定された会話が履歴に無ければ、別の会話に追記してしまわないよう送信自体を中止する。

起動時の自動再開は `index.ts` が行う。既定では行わず、白紙から始める(F-19)。
設定 `history.resumeLastOnStart` を有効にしたときだけ、最後のセッションを開き直す。
その識別子は `dataDir()/state.json` に持ち、設定にかかわらず記録し続ける。切り替えたとき即座に効くようにするためである。
...
|---|---|---|
| hello | - | 現在の `state` を要求 |
| prompt | requestId, text, sites, resumeSessionId? | 送信(F-03)。`resumeSessionId` があれば、送る前にタブをその会話へ戻す |
...
| session | session, resumed | 表示する会話。参照(`resumed: false`)は**要求したクライアントにだけ**返す。誰が何を見ているかは各クライアントの都合だからである。再開(`resumed: true`)はタブが動くので全クライアントへ配信し、接続時にも現在の会話を送る |
...
ただし、開いたセッションが現在の会話と同じなら参照中にしない。過去を見ているのではないからで、これを外すと「現在の会話に戻る」を押した利用者が入力できなくなる。
プロンプトを送るか、新しい会話を始めても参照中は解除される。

カラムの中身がまるごと差し替わったことは `viewEpoch` で伝える。
`session` と `clearTurns` で増やし、`SiteColumn` はこれが変わったときに最下部へスクロールする。
追記(`answer`)では増やさないので、生成中の追従スクロールと役割が分かれる。

WebSocket の close は、**新しい接続に切り替わった後に届くことがある**。
React の開発時の StrictMode は画面を二度組み立てるため、一つ目の接続が閉じるころには二つ目が生きている。
古い close で現在の接続を捨てないよう、閉じたのが今の接続かを確かめてから片付ける。
