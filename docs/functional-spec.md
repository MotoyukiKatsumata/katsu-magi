...
| `history.resumeLastOnStart` | true | 起動時に最後の会話を再開する |
...
**画面**は左の開閉できるパネルとする。
更新日で「今日」「昨日」「日付」の見出しを付け、新しい順に並べる。
各項目には題名、時刻、往復数を示し、現在の会話には印を付ける。
...
- **削除と名前変更**: 削除は katsu-magi の履歴からのみ消し、各サイト側の会話は残す。名前は題名のダブルクリックで変更する

**起動時の自動再開**。
サーバーは最後に使っていたセッションを記録しており、次の起動でそのセッションを再開する。
サーバーを再起動しても、追い質問を続けられる。
設定 `history.resumeLastOnStart` を `false` にすると、常に新しい会話から始める。
