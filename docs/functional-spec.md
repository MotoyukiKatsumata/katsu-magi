...
- サイトごとのモデル選択
- 画像や添付ファイルの送信
- 複数の katsu-magi を同時に開くこと(作業領域は常に一つ)
...
| F-19 | 会話履歴と再開 | やり取りを自動で保存し、後から参照する。過去の会話を各サイトで再開する |
...
利用者が入力欄にプロンプトを書き、Enter(または Send ボタン)で送信する。

キー操作は次のとおりとする。

| キー | 既定 | 「Enter で改行」にチェック |
|---|---|---|
| Enter | 送信 | 改行 |
| Shift+Enter | 改行 | 改行 |
| Alt+Enter | 改行 | 改行 |
| Ctrl+Enter(Cmd+Enter) | 送信 | 送信 |

Ctrl+Enter はどちらの設定でも送信とする。以前の版の操作をそのまま使えるようにするためで、他のキーと衝突しない。

日本語入力の変換確定に使う Enter では送信しない。
変換中の Enter は確定の操作であり、送信の意思ではないからである。

「Enter で改行」のチェックボックスは Send ボタンの上に置き、状態をブラウザに保存して次回も維持する。
...
- 過去の会話を参照中(F-19)に送信した場合は、送る前に各サイトのタブをその会話へ戻す。
...
対象の回答は、位置ではなく印で特定する。
送信の直前に、画面にあるすべての回答要素へ印を付け、以後は印の付いていない最後の要素を今回の回答として読む。
Gemini は画面外の古い回答を一覧から外すため、位置番号で数えると会話が長くなるにつれてずれる。
...
- 送信後、既定 120 秒たっても回答の器が現れない場合、利用上限のメッセージを探し(F-11)、見つからなければ「回答が現れない」エラーにする
- 器が一度でも現れた後は、この上限を適用しない。器ができた時点で送信は受け付けられており、その後の沈黙はモデルの思考だからである
- 既定 420 秒で完了しない場合、その時点までの内容を確定させたうえで「生成が完了しない」エラーにする
...
「New conversation」を押すと、有効な全サイトを新規チャットの URL に移動させる。
UI 上のターン履歴は消え、次のプロンプトが新しいセッション(F-19)を作る。
それまでの会話は履歴に残り、各サイト側の過去の会話も残る。
失われるものが無いので確認は求めない。
処理中(F-03)には実行できない。
...
| `timeouts.firstTokenMs` | 120000 | 回答の器が現れるまでの上限 |
| `timeouts.generationMs` | 420000 | 生成全体の上限 |
| `history.enabled` | true | 会話履歴を保存する(F-19) |
| `history.resumeLastOnStart` | false | 起動時に最後の会話を再開する。既定では新しい会話から始める |
...
やり取りは自動で保存される。利用者の操作は不要である。

**保存の単位**は「セッション」とする。
最初のプロンプトでセッションが作られ、以後のターンが追記される。
「新しい会話」(F-07)を押すと現在のセッションを離れ、次のプロンプトが新しいセッションを作る。
題名は最初のプロンプトの先頭 40 文字とし、利用者が変更できる。

セッションが保持するものは、各ターンのプロンプトとサイトごとの最終回答および状態、そして**サイトごとの会話 URL**である。
会話 URL は再開に使う。

**画面**は左の開閉できるパネルとする。
各項目には題名、時刻、往復数を示す。
パネルの開閉状態はブラウザに保存し、次回も同じ状態で開く。

並び順は**最後に使った順**とする。
順序が動くのはプロンプトを送ったときだけで、会話を開いただけ、名前を変えただけでは動かさない。
選んだ項目が勝手に先頭へ移ると、一覧の中での位置を覚えられず、探し直しになるからである。
見出しは更新日で「今日」「昨日」「日付」と区切る。

印は一つだけ付け、**いまカラムに表示している会話**を指す(参照中ならその会話)。
各サイトのタブがどの会話を開いているかは内部の事情であり、参照中に送信すれば表示中の会話の続きになる(F-03)ため、利用者に見せる意味がない。

**操作**は三つとする。

- **参照**: 題名をクリックすると、保存されたやり取りをカラムに表示する。各サイトのタブは動かさないので、待たずに読める。上部に帯を出し、送信するとその会話の続きになることを伝える
  - 表示した直後は、各カラムを**最後のやり取りが見える位置**にする。続きを読むのが目的であり、先頭から読み直すことは少ないからである
  - **プロンプト欄は使えるままにする**。参照中に送信した場合、送る直前に各サイトのタブをその会話へ戻してから送る。選んだら書ける、という期待に合わせるためで、読むだけのときにタブを動かす無駄も避けられる
  - 続ける会話が履歴から消えていた場合は、別の会話に書き込まないよう送信を中止し、その旨を通知する
  - 開いた相手が現在の会話であれば、帯を出さない。過去を見ているのではないからである
- **再開**: 各サイトのタブを保存済みの会話 URL へ移動し、そのセッションを現在の会話にする。以後の追い質問はその会話に続く。URL の無いサイトは新しいチャットにする。会話がサイト側で削除されていた場合は、そのサイトだけ「error」にし、他のサイトは続行する
- **削除と名前変更**: どちらも項目にカーソルを合わせると出るボタンから行う。削除は katsu-magi の履歴からのみ消し、各サイト側の会話は残す。名前を変えても一覧の順序は動かさない(下記)

**起動時は新しい会話から始める**。
各サイトのタブを新規チャットにし、カラムは空にする。
起動のたびに前回の続きが開くより、白紙から始めるほうが普段の使い方に合う。
続きを書きたい会話は履歴から選べばよく、ひと手間で済む。

設定 `history.resumeLastOnStart` を `true` にすると、最後に使っていたセッションを次の起動で再開する。
サーバーは最後のセッションを記録しているので、この設定だけで切り替わる。

再開した会話は、**カラムにも表示する**。
印だけが付いて中身が空だと、何が表示されているのか読み取れないからである。
画面を後から開いた場合も、そのとき開かれている会話を表示する。

保存先は `%LOCALAPPDATA%\katsu-magi\history\` とし、セッションごとに 1 ファイル、一覧用に索引を 1 ファイル置く。
索引は速さのための控えであり、壊れていればセッションのファイルから作り直す。
...
flowchart TB
    subgraph screen["katsu-magi 画面(ブラウザの横幅いっぱい)"]
        ...
        PB["プロンプト欄(複数行、Enter で送信) [Enter で改行] [Send / Cancel]"]
    end
    TB --> NT --> cols --> PB
```
