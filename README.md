...
1. 下部の入力欄にプロンプトを書き、**Enter**(または Send ボタン)で送信。改行は Shift+Enter か Alt+Enter です。
   入力欄の右上の「Enter で改行」にチェックを入れると、Enter が改行、Ctrl+Enter が送信になります。
2. 3 つのカラムに各サービスの回答が流れてきます。
3. そのまま追い質問すれば、各サイトの同じ会話に送られます。
4. カラム上部のチェックを外すと、そのサイトには送りません。
5. **Copy** で最新回答を Markdown としてコピーできます。
6. カラムの間の仕切りをドラッグすると幅を変えられます。ダブルクリックで三等分に戻ります。

### 履歴と再開

やり取りは自動で保存されます。左の「履歴」パネルに会話が一覧で出ます。

- 題名をクリックすると、その会話を**参照**できます(各サイトのタブは動きません)。参照中はプロンプト欄が無効になります
- 「再開」を押すと、3 サイトのタブが当時の会話に戻り、**続きから追い質問**できます
